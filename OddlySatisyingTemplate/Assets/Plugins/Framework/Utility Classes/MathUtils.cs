...
namespace Framework
{
    ...
    public static class MathUtils
    {
        ...
        {
            Vector3 ab = a.sqrMagnitude >= b.sqrMagnitude ? a : b;
            return ab.sqrMagnitude >= c.sqrMagnitude ? ab : c;
        }

        public static Vector3 Max(params Vector3[] vectors)
        {
            float max = vectors[0].sqrMagnitude;
            Vector3 result = vectors[0];

            for (int i = 1; i < vectors.Length; i++)
            {
                float mag = vectors[i].sqrMagnitude;
                if (mag > max)
                {
                    max = mag;
                    result = vectors[i];
        ...
        {
            return Mathf.Asin(opposite / hypotenuse) * Mathf.Rad2Deg;
        }

        public static float AngleFromAdjacentAndHypotenuse(float adjacent, float hypotenuse)
        {
            return Mathf.Acos(adjacent / hypotenuse) * Mathf.Rad2Deg;
        }
        ...
        public static void GetComponentAngles(Vector3 from, Vector3 to, out float horizontalAngle, out float verticalAngle)
        {
            if (from == Vector3.zero || to == Vector3.zero || from == to)
            {
                horizontalAngle = 0f;
                verticalAngle = 0f;
            }
            ...
            {
                Vector3 local = Quaternion.Inverse(Quaternion.LookRotation(from)) * to;
                horizontalAngle = Mathf.Abs(Mathf.Atan(local.x / local.z) * Mathf.Rad2Deg);
                verticalAngle = Mathf.Abs(Mathf.Atan(local.y / local.WithY(0).magnitude) * Mathf.Rad2Deg);
            }
        }


        public static void GetComponentAnglesSigned(Vector3 from, Vector3 to, out float horizontalAngle, out float verticalAngle)
        {
            if (from == Vector3.zero || to == Vector3.zero || from == to)
            {
                horizontalAngle = 0f;
            ...
            {
                Vector3 local = Quaternion.Inverse(Quaternion.LookRotation(from)) * to;
                horizontalAngle = Mathf.Atan(local.x / local.z) * Mathf.Rad2Deg;
                verticalAngle = Mathf.Atan(local.y / local.WithY(0).magnitude) * Mathf.Rad2Deg;
            }
        }

        public static float Sin2(float x)
        {
            return 0.5f * (1f - Mathf.Cos(2f * x));
        }
        ...
        public static float Tan2(float x)
        {

            return (1f - Mathf.Cos(2f * x)) / (1f + Mathf.Cos(2f * x));
        }
        ...
        {
            CalculateDampedSpringMotionParameters(dampingRatio, angularFrequency, Time.deltaTime, out float pp, out float pv, out float vp, out float vv);
        ...
        public static void SimpleHarmonicMotion(ref float currentValue, ref float velocity, float targetValue, float pp, float pv, float vp, float vv)
        {
        ...
        public static void CalculateDampedSpringMotionParameters(float dampingRatio, float angularFrequency, float deltaTime, out float pp, out float pv, out float vp, out float vv)
        {
        ...
        public static Color HCLApproach(Color from, Color to, float speed)
        {
            return HCLColour.Lerp(from, to, 1 - Mathf.Exp(-speed * Time.deltaTime)).ToRGB();
        }
        ...
        public static float CalculateMaxProjectileRange(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity)
        {
            return CalculateMaxProjectileRange(launchPosition, targetPosition, launchVelocity, Physics.gravity.y);
        }

        public static float CalculateMaxProjectileRange(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity, float gravity)
        {
            float v = launchVelocity;
            float g = Mathf.Abs(gravity);
            float y = launchPosition.y - targetPosition.y;
            ...
            float ta = gy + v2;
            float tb = gy + 2 * v2;
            float theta = Mathf.Acos(Mathf.Sqrt(ta / tb));

            float a = (v * Mathf.Cos(theta)) / g;
            float b = v * Mathf.Sin(theta);
            float c = v2 * Sin2(theta);

            return a * (b + Mathf.Sqrt(c + gy));
        }

        public static float CalculateMaxRangeProjectileAngle(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity)
        {
            return CalculateMaxRangeProjectileAngle(launchPosition, targetPosition, launchVelocity, Physics.gravity.y);
        }

        public static float CalculateMaxRangeProjectileAngle(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity, float gravity)
        {
            float v2 = launchVelocity * launchVelocity;
            float gy = 2f * Mathf.Abs(gravity) * (launchPosition.y - targetPosition.y);

            float ta = gy + v2;
            float tb = gy + 2 * v2;

            return Mathf.Acos(Mathf.Sqrt(ta / tb)) * Mathf.Rad2Deg;
        }

        public static Vector3 CalculateMaxRangeProjectileLaunchVelocity(Vector3 launchPosition, Vector3 targetPosition, float launchVelocityMagnitude)
        {
            return CalculateMaxRangeProjectileLaunchVelocity(launchPosition, targetPosition, launchVelocityMagnitude, Physics.gravity.y);
        }

        public static Vector3 CalculateMaxRangeProjectileLaunchVelocity(Vector3 launchPosition, Vector3 targetPosition, float launchVelocityMagnitude, float gravity)
        {
            Vector3 toTarget = (targetPosition - launchPosition).WithY(0).normalized;
            Vector3 axis = Vector3.Cross(toTarget, Vector3.up);
            return Quaternion.AngleAxis(CalculateMaxRangeProjectileAngle(launchPosition, targetPosition, launchVelocityMagnitude, gravity), axis) * toTarget * launchVelocityMagnitude;
        }

        public static Vector3 CalculateProjectileLaunchVelocity(Vector3 launchPosition, Vector3 targetPosition, float launchVelocityMagnitude)
        {
            return CalculateProjectileLaunchVelocity(launchPosition, targetPosition, launchVelocityMagnitude, Physics.gravity.y);
        }

        public static float CalculateProjectileLaunchAngle(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity)
        {
            return CalculateProjectileLaunchAngle(launchPosition, targetPosition, launchVelocity, Physics.gravity.y);
        }

        public static Vector3 CalculateProjectileLaunchVelocity(Vector3 launchPosition, Vector3 targetPosition, float launchVelocityMagnitude, float gravity)
        {
            Vector3 toTarget = (targetPosition - launchPosition).WithY(0).normalized;
            Vector3 axis = Vector3.Cross(toTarget, Vector3.up);
            return Quaternion.AngleAxis(CalculateProjectileLaunchAngle(launchPosition, targetPosition, launchVelocityMagnitude, gravity), axis) * toTarget * launchVelocityMagnitude;
        }

        public static float CalculateProjectileLaunchAngle(Vector3 launchPosition, Vector3 targetPosition, float launchVelocity, float gravity)
        {
            ...
            float numerator = v2 - Mathf.Sqrt(v4 - ((g * g * x2) + (2 * y * v2 * g)));
            float denominator = g * x;

            return Mathf.Rad2Deg * Mathf.Atan(numerator / denominator);
        }

        public static Vector3 CalculateProjectilePositionAtTime(Vector3 launchPosition, Vector3 launchVelocity, float time)
        {
            return launchPosition + (launchVelocity * time) + (0.5f * Physics.gravity * time * time);
        }

        public static Vector3 CalculateProjectilePositionAtTime(Vector3 launchPosition, Vector3 launchVelocity, float time, Vector3 gravity)
        {
            return launchPosition + (launchVelocity * time) + (0.5f * gravity * time * time);
        }

        public static Vector3[] CalculateProjectileArc(Vector3 launchPosition, Vector3 launchVelocity, float time)
        {

            return CalculateProjectileArc(launchPosition, launchVelocity, time, Physics.gravity);
        }

        public static Vector3[] CalculateProjectileArc(Vector3 launchPosition, Vector3 launchVelocity, float time, Vector3 gravity)
        {
            List<Vector3> points = new List<Vector3>();
            float t = 0f;
            while (t < time)
            {
                points.Add(launchPosition + (launchVelocity * t) + (0.5f * gravity * t * t));
